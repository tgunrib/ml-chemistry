...
# trains the model using global descriptors and clustering
...
model = AttentiveFP(in_channels=72, hidden_channels=200, out_channels=1,
...
def train(train_loader):
    ...
    for data in train_loader:
        data = data.to(device)
        optimizer.zero_grad()
        out = model(data.x_combined, data.edge_index, data.edge_attr, data.batch)
